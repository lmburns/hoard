...
/// A conditional structure that checks against the operating system `hoard` was
/// compiled for.
///
/// This has the effect of "detecting" the operating system at compile time
/// instead of runtime. The downside is that running `hoard` in [Wine](https://www.winehq.org/) will detect the system
/// as Windows, while running in the Windows Subsystem for Linux or FreeBSD's
/// Linuxulator will detect the system as Linux.
///
