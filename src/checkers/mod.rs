...
//!
//! This module includes a single trait, [`Checker`], and all types that
//! implement it. Currently, that is only the
//! [`LastPaths`](history::last_paths::LastPaths) checker.
...
///
/// A [`Checker`] takes a [`Hoard`] and its name (as [`&str`]) as parameters and
/// uses that information plus any internal state to validate that it is safe to
/// operate on that [`Hoard`].
pub trait Checker: Sized {
    ...
    /// # Errors
    ///
    /// Any errors that may occur while creating an instance, such as I/O or
    /// consistency errors.
    ...
    /// # Errors
    ///
    /// Any error that prevents operations on the given [`Hoard`], or any errors
    /// that occur while performing the check.
