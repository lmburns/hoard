...
//!
//! The types in this module are used for logging all operations to disk. This
//! information can be used for debugging purposes, but is more directly used as
//! a [`Checker`] to help prevent synchronized changes from being overwritten.
//!
//! It does this by parsing synchronized logs from this and other systems to
//! determine which system was the last one to touch a file.

use crate::{
    checkers::Checker,
    config::hoard::{Hoard as ConfigHoard, Pile as ConfigPile},
};
use md5::{Digest, Md5};
...
pub enum Error {
    ...
    /// There are unapplied changes from another system. A restore or forced
    /// backup is required.
    #[error(
        "found unapplied remote changes - restore this hoard to apply changes or force a backup \
         with --force"
    )]
    RestoreRequired,
}
...
///
/// This keeps track of the timestamp of the operation (which may include
/// multiple hoards), all hoards involved in the operation (and the related
/// [`HoardOperation`]), and a record of the latest operation log for each
/// external system at the time of invocation.
...
impl HoardOperation {
    ...
    /// # Errors
    ///
    /// Returns [`Error::RestoreRequired`] if they do not have the same files
    /// (and hashes).
    ...
    /// Returns the latest operation for the given hoard from a system history
    /// root directory.
    ...
    /// Returns the latest backup operation recorded on any other machine (by
    /// UUID).
    ///
    /// # Errors
    ///
