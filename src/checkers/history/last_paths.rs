...
//!
//! See the documentation for [`HoardPaths::enforce_old_and_new_piles_are_same`]
...
pub enum Error {
    ...
    IO(#[from] io::Error),
    /// Unexpected differences in hoard paths. Operation must be forced to
    /// continue.
...
impl LastPaths {
    ...
    /// The exception is an I/O error with kind `NotFound`, which returns an
    /// empty `LastPaths`.
...
impl HoardPaths {
    ...
    ///
    /// This check basically returns an error if `old != new`, but does some
    /// extra checking to provided better logged warnings depending on what
    /// the mismatch is.
    ///
    /// Any mismatch is considered an inconsistency because it may mean that one
    /// of the following occurs because of an unexpected change in the
    /// system configuration:
    ///
    /// - Backing up an empty/non-existent directory unintentionally deletes the
    ///   current backup.
    /// - Restoring to an unexpected directory causes nothing to change in the
    ///   intended one.
    /// - Files don't get backed up or restored because the system configuration
    ///   did not match a pile's environment string.
    ///
    /// # Example
    ///
    /// I sometimes run a normal Steam installation on a Linux machine,
    /// sometimes the flatpak version. If the flatpak install takes priority
    /// over the normal install, the following set of events might occur:
    ///
    /// - Install flatpak Steam to try it out.
    /// - Realize I should back up my normal Steam saves so I can restore them
    ///   to the flatpak locations.
    ///   - Alternatively, I told `hoard` to do this already and assumed it
    ///     finished, but a different hoard took longer than expected to back
    ///     up.
    ///   - Either way, a backup happens after installing flatpak Steam.
    /// - Existing backup is erased so the new one is a clean backup.
    /// - The associated directories for flatpak are empty or don't exist, so
    ///   nothing gets backed up.
    /// - My backups are all deleted.
    ///
    /// There are ways to recover from this, for example by uninstalling flatpak
    /// Steam and doing another backup, but the situation gets more complex
    /// when considering multiple devices each synchronizing files to each
    /// other. It's much easier to make this check and, if the changes are
    /// intended, have the user indicate as much.
    ///
    /// # Errors
    ///
