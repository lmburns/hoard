...
// file.reopen().expect("failed to reopen file after writing");
//
// toml::assert_file_contains_deserializable(file.path(),
// &strip_non_config_file(expected));
//
// Cleanup
// std::fs::remove_file(file.path()).expect("failed to delete file");
// }
...
//
// let builder =
// ConfigBuilder::from(run_config.clone()).set_saves_root(saves_root);
//
// run_config
// .command
// .run(&run_config)
